...
/**
 * ArrayOperations provides simple operation that return the prime numbers of a file.
 * 
 * @author AthinaDavari
 *
 */
public class ArrayOperations {
	...
	/**
	 * This function generates an array of numbers from a file, and returns a new array that contains only its
	 * prime numbers of the array with the file inputs.
	 * 
	...
	int[] findPrimesInFile(FileIO fileIo, String filepath, MyMath myMath) {
		...
		return data.stream().filter(x -> myMath.isPrime(x)).mapToInt(i -> i).toArray();
	}
}
